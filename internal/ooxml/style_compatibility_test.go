...
func TestDocxInheritedEmphasisKeepsWhitespaceOutsideDelimiters(t *testing.T) {
	styles := stylePart(`<w:style w:type="paragraph" w:default="true" w:styleId="Normal"><w:rPr><w:b/></w:rPr></w:style>`)
	for _, tc := range []struct{ name, runs, want string }{
		{"format boundary", `<w:r><w:t>Bold </w:t></w:r><w:r><w:rPr><w:b w:val="0"/></w:rPr><w:t>plain</w:t></w:r>`, "**Bold** plain\n"},
		{"matching formatting", `<w:r><w:t>Bold </w:t></w:r><w:r><w:t>again</w:t></w:r>`, "**Bold again**\n"},
		{"trailing whitespace", `<w:r><w:t>Bold </w:t></w:r>`, "**Bold**\n"},
		{"whitespace run", `<w:r><w:t>Bold</w:t></w:r><w:r><w:t> </w:t></w:r><w:r><w:rPr><w:b w:val="0"/></w:rPr><w:t>plain</w:t></w:r>`, "**Bold** plain\n"},
		{"code has no empty emphasis", `<w:r><w:rPr><w:rFonts w:ascii="Consolas"/></w:rPr><w:t>code</w:t></w:r>`, "`code`\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			doc := convertDocxBody(t, `<w:p>`+tc.runs+`</w:p>`, styles)
			if doc.Markdown != tc.want {
				t.Fatalf("Markdown = %q, want %q", doc.Markdown, tc.want)
			}
		})
...
func TestDocxInheritedCodePreservesPunctuation(t *testing.T) {
	styles := stylePart(`<w:style w:type="paragraph" w:default="true" w:styleId="Code"><w:rPr><w:rFonts w:ascii="Consolas"/></w:rPr></w:style>`)
	for _, tc := range []struct{ text, want string }{
		...
		t.Run(tc.text, func(t *testing.T) {
			doc := convertDocxBody(t, `<w:p><w:r><w:t>`+tc.text+`</w:t></w:r></w:p>`, styles)
			if doc.Markdown != tc.want {
				t.Fatalf("Markdown = %q, want %q", doc.Markdown, tc.want)
			}
		})
