...
func TestHiddenDrawingImagesAreOmitted(t *testing.T) {
	for _, format := range []string{"pptx", "pptx-shape", "xlsx"} {
		t.Run(format, func(t *testing.T) {
			parts := imagePackage(format, svgBlip(`r:embed="rRaster"`), false)
			for i := range parts {
			...
			if len(doc.Images) != 0 || strings.Contains(doc.Markdown, "![") {
				t.Fatalf("hidden image emitted: %#v", doc)
			}
		})
	}
}
...
func imagePackage(format, blip string, raster bool) []testPackagePart {
	...
	case "pptx", "pptx-shape":
		...
		parts = append(parts,
			testPackagePart{name: main, text: `<p:presentation` + ns + `><p:sldIdLst><p:sldId id="256" r:id="rSlide"/></p:sldIdLst></p:presentation>`},
			testPackagePart{name: "ppt/_rels/presentation.xml.rels", text: `<Relationships xmlns="` + packageRelsNS + `"><Relationship Id="rSlide" Type="` + officeRelsNS + `/slide" Target="slides/slide1.xml"/></Relationships>`},
	...
	case "xlsx":
		...
		parts = append(parts,
			testPackagePart{name: main, text: `<workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"` + ns + `><sheets><sheet name="Images" r:id="rSheet"/></sheets></workbook>`},
			...
			testPackagePart{name: "xl/worksheets/sheet1.xml", text: `<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"` + ns + `><sheetData/><drawing r:id="rDrawing"/></worksheet>`},
